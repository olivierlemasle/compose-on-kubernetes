...
func (u *k8sResourceUpdater) applyDaemonsets(toAdd, toUpdate, toDelete []appstypes.DaemonSet) error {
	for _, r := range toDelete {
		if err := u.k8sclient.AppsV1().DaemonSets(u.originalStack.Namespace).Delete(context.TODO(), r.Name, *&deleteOptions); err != nil && !kerrors.IsNotFound(err) {
	...
	for _, r := range toAdd {
		if _, err := u.k8sclient.AppsV1().DaemonSets(u.originalStack.Namespace).Create(context.TODO(), &r, metav1.CreateOptions{}); err != nil {
	...
	for _, r := range toUpdate {
		if _, err := u.k8sclient.AppsV1().DaemonSets(u.originalStack.Namespace).Update(context.TODO(), &r, metav1.UpdateOptions{}); err != nil {
...
func (u *k8sResourceUpdater) applyDeployments(toAdd, toUpdate, toDelete []appstypes.Deployment) error {
	for _, r := range toDelete {
		if err := u.k8sclient.AppsV1().Deployments(u.originalStack.Namespace).Delete(context.TODO(), r.Name, *&deleteOptions); err != nil && !kerrors.IsNotFound(err) {
	...
	for _, r := range toAdd {
		if _, err := u.k8sclient.AppsV1().Deployments(u.originalStack.Namespace).Create(context.TODO(), &r, metav1.CreateOptions{}); err != nil {
	...
	for _, r := range toUpdate {
		if _, err := u.k8sclient.AppsV1().Deployments(u.originalStack.Namespace).Update(context.TODO(), &r, metav1.UpdateOptions{}); err != nil {
...
func (u *k8sResourceUpdater) applyStatefulsets(toAdd, toUpdate, toDelete []appstypes.StatefulSet) error {
	for _, r := range toDelete {
		if err := u.k8sclient.AppsV1().StatefulSets(u.originalStack.Namespace).Delete(context.TODO(), r.Name, *&deleteOptions); err != nil && !kerrors.IsNotFound(err) {
	...
	for _, r := range toAdd {
		if _, err := u.k8sclient.AppsV1().StatefulSets(u.originalStack.Namespace).Create(context.TODO(), &r, metav1.CreateOptions{}); err != nil {
	...
	for _, r := range toUpdate {
		if _, err := u.k8sclient.AppsV1().StatefulSets(u.originalStack.Namespace).Update(context.TODO(), &r, metav1.UpdateOptions{}); err != nil {
...
func (u *k8sResourceUpdater) applyServices(toAdd, toUpdate, toDelete []coretypes.Service) error {
	for _, r := range toDelete {
		if err := u.k8sclient.CoreV1().Services(u.originalStack.Namespace).Delete(context.TODO(), r.Name, *&deleteOptions); err != nil && !kerrors.IsNotFound(err) {
	...
	for _, r := range toAdd {
		if _, err := u.k8sclient.CoreV1().Services(u.originalStack.Namespace).Create(context.TODO(), &r, metav1.CreateOptions{}); err != nil {
	...
	for _, r := range toUpdate {
		if _, err := u.k8sclient.CoreV1().Services(u.originalStack.Namespace).Update(context.TODO(), &r, metav1.UpdateOptions{}); err != nil {
...
func (u *k8sResourceUpdater) deleteSecretsNoCollection() error {
	...
	for name, s := range u.originalStack.Spec.Secrets {
		...
		if err := u.k8sclient.CoreV1().Secrets(u.originalStack.Namespace).Delete(context.TODO(), name, metav1.DeleteOptions{}); err != nil && !kerrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (u *k8sResourceUpdater) deleteConfigMapsNoCollection() error {
	...
	for name, s := range u.originalStack.Spec.Configs {
		...
		if err := u.k8sclient.CoreV1().ConfigMaps(u.originalStack.Namespace).Delete(context.TODO(), name, metav1.DeleteOptions{}); err != nil && !kerrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (u *k8sResourceUpdater) deleteSecretsAndConfigMaps() error {
	...
	if err := u.k8sclient.CoreV1().Secrets(u.originalStack.Namespace).DeleteCollection(context.TODO(), metav1.DeleteOptions{}, listOptions); err != nil {
	...
	if err := u.k8sclient.CoreV1().ConfigMaps(u.originalStack.Namespace).DeleteCollection(context.TODO(), metav1.DeleteOptions{}, listOptions); err != nil {
		if kerrors.IsForbidden(err) {
